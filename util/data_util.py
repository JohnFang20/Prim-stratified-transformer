...
def collate_fn_limit(batch, max_batch_points, logger):
    coord, normals, boundary, label, semantic, param = list(zip(*batch))
    offset, count = [], 0
    # print("coord:", len(coord))
    k = 0
    for item in coord:
    ...
    return torch.cat(coord[:k]), torch.cat(normals[:k]), torch.cat(boundary[:k]), torch.cat(label[:k]), torch.cat(semantic[:k]), torch.cat(param[:k]), torch.IntTensor(offset[:k])
    # return torch.cat(coord), torch.cat(feat), torch.cat(label), torch.IntTensor(offset)
...
def collate_fn(batch):
    ...
    for item in coord:
        ...
        count += item.shape[0]
        offset.append(count)
    return torch.cat(coord), torch.cat(normals), torch.cat(boundary), torch.cat(label), torch.cat(semantic), torch.cat(param), torch.IntTensor(offset)
...
def data_prepare_abc(coord, normals, boundary, label, semantic, param, split='train', voxel_size=0.04, voxel_max=None, transform=None, shuffle_index=False):
    # #     if transform:
    #     # coord, feat, label = transform(coord, feat, label)
    #     coord, feat = transform(coord, feat)
    if voxel_size:
        ...
        coord -= coord_min
        uniq_idx = voxelize(coord, voxel_size)
        coord, normals, boundary, label, semantic, param = coord[uniq_idx], normals[uniq_idx], boundary[uniq_idx], label[uniq_idx], semantic[uniq_idx], param[uniq_idx]
    # if voxel_max and label.shape[0] > voxel_max:
    #     init_idx = np.random.randint(label.shape[0]) if 'train' in split else label.shape[0] // 2
    #     crop_idx = np.argsort(np.sum(np.square(coord - coord[init_idx]), 1))[:voxel_max]
    #     coord, feat, label = coord[crop_idx], feat[crop_idx], label[crop_idx]
    ...
    label = torch.LongTensor(label)
    return coord, normals, boundary, label, semantic, param
