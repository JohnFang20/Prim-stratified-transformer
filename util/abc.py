...
class ABC_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        coord, normals, boundary, label, semantic, param = data_prepare_abc(coord, normals, boundary, label, semantic, param, voxel_size=self.voxel_size)
...
if __name__ == '__main__':
    ...
    train_loader = torch.utils.data.DataLoader(point_data, batch_size=2, shuffle=False, num_workers=0, pin_memory=True, collate_fn=collate_fn)
    for idx in range(1):
        end = time.time()
        voxel_num = []
        for i, (coord, normals, boundary, label, semantic, param, offset) in enumerate(train_loader):
